...
function -fn-copy() {
    ...
    for arg in "$@"; do
        case $arg in
            ...
            --help)
                echo "Usage: ${FUNCNAME[0]} [OPTION]... OLD_FUNC_NAME NEW_FUNC_NAME"
                echo 
    ...
    if [[ $_OLD_FUNC_NAME ]]; then
        ...
        echo "Error: Parameter OLD_FUNC_NAME must be specified."; return 1
    fi
    if [[ $_NEW_FUNC_NAME ]]; then
        true
    else
        echo "Error: Parameter NEW_FUNC_NAME must be specified."; return 1
    fi
...
function -fn-exists() {
    ...
    for arg in "$@"; do
        case $arg in
            ...
            --help)
                echo "Usage: ${FUNCNAME[0]} [OPTION]... FUNC_NAME"
                echo 
                ...
                echo 
                echo "Examples:"
                echo -e "$ \e[1m${FUNCNAME[0]} name-of-nonexistant-function\e[22m"
                echo 
                echo -e "$ \e[1m${FUNCNAME[0]} -v name-of-nonexistant-function\e[22m"
            ...
            --selftest)
                ...
                echo "--> OK"
                echo -e "$ \e[1m${FUNCNAME[0]} name-of-nonexistant-function\e[22m"
                stdout=$(${FUNCNAME[0]} name-of-nonexistant-function); rc=$?
                echo $stdout
                ...
                echo "--> OK"
                echo -e "$ \e[1m${FUNCNAME[0]} -v name-of-nonexistant-function\e[22m"
                stdout=$(${FUNCNAME[0]} -v name-of-nonexistant-function); rc=$?
                echo $stdout
                if [[ $rc != 1 ]]; then echo "--> FAILED - exit code [$rc] instead of expected [1]."; return 1; fi
                regex="^A function with the name 'name-of-nonexistant-function' does not exist.$"
                if [[ ! "$stdout" =~ $regex ]]; then echo "--> FAILED - stdout [$stdout] does not match required pattern [A function with the name 'name-of-nonexistant-function' does not exist.]."; return 1; fi
                echo "--> OK"
                echo "Testing function [${FUNCNAME[0]}]...DONE"
                return 0
              ;;
    ...
    if [[ $_FUNC_NAME ]]; then
        ...
        echo "Error: Parameter FUNC_NAME must be specified."; return 1
    fi
