...
function -block-port() {
    ...
    for arg in "$@"; do
        case $arg in
            ...
            --help)
                echo "Usage: ${FUNCNAME[0]} [OPTION]... [BIND_ADDRESS] PORT"
                echo 
                ...
                echo 
                echo "Examples:"
                echo -e "$ \e[1m${FUNCNAME[0]} 70000\e[22m"
            ...
            --selftest)
                ...
                if [[ ! "$stdout" =~ $regex ]]; then echo "--> FAILED - stdout [$stdout] does not match required pattern [Error: Value '70000' for parameter PORT is too high. Must be <= 65535.]."; return 1; fi
                echo "--> OK"
    ...
    if [[ $_PORT ]]; then
        if [[ ! "$_PORT" =~ ^-?[0-9]*$ ]]; then echo "Error: Value '$_PORT' for parameter PORT is not a numeric value."; return 1; fi
        if [[ $_PORT -lt 0 ]]; then echo "Error: Value '$_PORT' for parameter PORT is too low. Must be >= 0."; return 1; fi
        if [[ $_PORT -gt 65535 ]]; then echo "Error: Value '$_PORT' for parameter PORT is too high. Must be <= 65535."; return 1; fi
        true
    else
        echo "Error: Parameter PORT must be specified."; return 1
    fi
...
function -is-port-open() {
    ...
    for arg in "$@"; do
        case $arg in
            ...
            --help)
                echo "Usage: ${FUNCNAME[0]} [OPTION]... HOSTNAME PORT [CONNECT_TIMEOUT_IN_SECONDS]"
                echo 
                ...
                echo 
                echo "Examples:"
                echo -e "$ \e[1m${FUNCNAME[0]} localhost 12345 1\e[22m"
                echo 
                echo -e "$ \e[1m${FUNCNAME[0]} -v localhost 12345 1\e[22m"
                echo "localhost:12345 is not reachable."
                echo -e "$ \e[1m${FUNCNAME[0]} localhost 70000\e[22m"
                echo "Error: Value '70000' for parameter PORT is too high. Must be <= 65535."
                echo 
                return 0
              ;;
            ...
            --selftest)
                ...
                echo "--> OK"
                echo -e "$ \e[1m${FUNCNAME[0]} localhost 12345 1\e[22m"
                stdout=$(${FUNCNAME[0]} localhost 12345 1); rc=$?
                echo $stdout
                if [[ $rc != 1 ]]; then echo "--> FAILED - exit code [$rc] instead of expected [1]."; return 1; fi
                regex="^$"
                ...
                echo "--> OK"
                echo -e "$ \e[1m${FUNCNAME[0]} -v localhost 12345 1\e[22m"
                stdout=$(${FUNCNAME[0]} -v localhost 12345 1); rc=$?
                echo $stdout
                if [[ $rc != 1 ]]; then echo "--> FAILED - exit code [$rc] instead of expected [1]."; return 1; fi
                regex="^localhost:12345 is not reachable.$"
                if [[ ! "$stdout" =~ $regex ]]; then echo "--> FAILED - stdout [$stdout] does not match required pattern [localhost:12345 is not reachable.]."; return 1; fi
                echo "--> OK"
                echo -e "$ \e[1m${FUNCNAME[0]} localhost 70000\e[22m"
                stdout=$(${FUNCNAME[0]} localhost 70000); rc=$?
                echo $stdout
                if [[ $rc != 1 ]]; then echo "--> FAILED - exit code [$rc] instead of expected [1]."; return 1; fi
                regex="^Error: Value '70000' for parameter PORT is too high. Must be <= 65535.$"
                if [[ ! "$stdout" =~ $regex ]]; then echo "--> FAILED - stdout [$stdout] does not match required pattern [Error: Value '70000' for parameter PORT is too high. Must be <= 65535.]."; return 1; fi
    ...
    if [[ $_HOSTNAME ]]; then
        ...
        echo "Error: Parameter HOSTNAME must be specified."; return 1
    fi
    if [[ $_PORT ]]; then
        if [[ ! "$_PORT" =~ ^-?[0-9]*$ ]]; then echo "Error: Value '$_PORT' for parameter PORT is not a numeric value."; return 1; fi
        if [[ $_PORT -lt 0 ]]; then echo "Error: Value '$_PORT' for parameter PORT is too low. Must be >= 0."; return 1; fi
    ...
    if [[ $_CONNECT_TIMEOUT_IN_SECONDS ]]; then
        if [[ ! "$_CONNECT_TIMEOUT_IN_SECONDS" =~ ^-?[0-9]*$ ]]; then echo "Error: Value '$_CONNECT_TIMEOUT_IN_SECONDS' for parameter CONNECT_TIMEOUT_IN_SECONDS is not a numeric value."; return 1; fi
        true
    fi
