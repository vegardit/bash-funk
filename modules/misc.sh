...
function -var-exists() {
    ...
    for arg in "$@"; do
        case $arg in
            ...
            --help)
                echo "Usage: ${FUNCNAME[0]} [OPTION]... VARIABLE_NAME"
                echo 
                ...
                echo 
                echo "Examples:"
                echo -e "$ \e[1m${FUNCNAME[0]} USER\e[22m"
                echo 
                echo -e "$ \e[1m${FUNCNAME[0]} -v USER\e[22m"
                echo "Bash variable 'USER' is exists."
                echo -e "$ \e[1m${FUNCNAME[0]} -v NON_EXISTANT_VARIABLE\e[22m"
            ...
            --selftest)
                ...
                echo "--> OK"
                echo -e "$ \e[1m${FUNCNAME[0]} -v USER\e[22m"
                stdout=$(${FUNCNAME[0]} -v USER); rc=$?
                echo $stdout
                if [[ $rc != 0 ]]; then echo "--> FAILED - exit code [$rc] instead of expected [0]."; return 1; fi
                regex="^Bash variable 'USER' is exists.$"
                if [[ ! "$stdout" =~ $regex ]]; then echo "--> FAILED - stdout [$stdout] does not match required pattern [Bash variable 'USER' is exists.]."; return 1; fi
                echo "--> OK"
                echo -e "$ \e[1m${FUNCNAME[0]} -v NON_EXISTANT_VARIABLE\e[22m"
                stdout=$(${FUNCNAME[0]} -v NON_EXISTANT_VARIABLE); rc=$?
                echo $stdout
                ...
                if [[ ! "$stdout" =~ $regex ]]; then echo "--> FAILED - stdout [$stdout] does not match required pattern [Bash variable 'NON_EXISTANT_VARIABLE' does not exist.]."; return 1; fi
                echo "--> OK"
                echo "Testing function [${FUNCNAME[0]}]...DONE"
                return 0
              ;;
    ...
    if [[ $_VARIABLE_NAME ]]; then
        ...
        echo "Error: Parameter VARIABLE_NAME must be specified."; return 1
    fi
...
function -wait() {
    ...
    for arg in "$@"; do
        case $arg in
            ...
            --help)
                echo "Usage: ${FUNCNAME[0]} [OPTION]... SECONDS"
                echo 
    ...
    if [[ $_SECONDS ]]; then
        ...
        echo "Error: Parameter SECONDS must be specified."; return 1
    fi
