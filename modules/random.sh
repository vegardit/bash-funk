...
function -random-number() {
    ...
    for arg in "$@"; do
        case $arg in
            ...
            --help)
                echo "Usage: ${FUNCNAME[0]} [OPTION]... RANGE"
                echo 
                ...
                echo 
                echo "Examples:"
                echo -e "$ \e[1m${FUNCNAME[0]} 1-1\e[22m"
                echo "1"
                echo -e "$ \e[1m${FUNCNAME[0]} 1-5\e[22m"
                echo "4"
                echo -e "$ \e[1m${FUNCNAME[0]} 200-299\e[22m"
                echo "253"
                echo 
                return 0
              ;;
            ...
            --selftest)
                ...
                echo "--> OK"
                echo -e "$ \e[1m${FUNCNAME[0]} 1-1\e[22m"
                stdout=$(${FUNCNAME[0]} 1-1); rc=$?
                echo $stdout
                if [[ $rc != 0 ]]; then echo "--> FAILED - exit code [$rc] instead of expected [0]."; return 1; fi
                regex="^1$"
                if [[ ! "$stdout" =~ $regex ]]; then echo "--> FAILED - stdout [$stdout] does not match required pattern [1]."; return 1; fi
                echo "--> OK"
                echo -e "$ \e[1m${FUNCNAME[0]} 1-5\e[22m"
                stdout=$(${FUNCNAME[0]} 1-5); rc=$?
                echo $stdout
                if [[ $rc != 0 ]]; then echo "--> FAILED - exit code [$rc] instead of expected [0]."; return 1; fi
                regex="^[1-5]$"
                if [[ ! "$stdout" =~ $regex ]]; then echo "--> FAILED - stdout [$stdout] does not match required pattern [[1-5]]."; return 1; fi
                echo "--> OK"
                echo -e "$ \e[1m${FUNCNAME[0]} 200-299\e[22m"
                stdout=$(${FUNCNAME[0]} 200-299); rc=$?
                echo $stdout
                if [[ $rc != 0 ]]; then echo "--> FAILED - exit code [$rc] instead of expected [0]."; return 1; fi
                regex="^[2-3][0-9][0-9]$"
                if [[ ! "$stdout" =~ $regex ]]; then echo "--> FAILED - stdout [$stdout] does not match required pattern [[2-3][0-9][0-9]]."; return 1; fi
                echo "--> OK"
    ...
    if [[ $_RANGE ]]; then
        ...
        if [[ ! "$_RANGE" =~ $regex ]]; then echo "Error: Value '$_RANGE' for parameter RANGE does not match required pattern '[1-9][0-9]*-[1-9][0-9]*'."; return 1; fi
        unset regex
        true
    else
        echo "Error: Parameter RANGE must be specified."; return 1
    fi
...
function -random-string() {
    ...
    for arg in "$@"; do
        case $arg in
            ...
            --help)
                echo "Usage: ${FUNCNAME[0]} [OPTION]... LENGTH [CHARS]"
                echo 
                ...
                echo 
                echo "Examples:"
                echo -e "$ \e[1m${FUNCNAME[0]} 12 0-9\e[22m"
                echo "686026902293"
                echo -e "$ \e[1m${FUNCNAME[0]} 8 a-zA-Z\e[22m"
                echo "ECtQptCz"
                echo -e "$ \e[1m${FUNCNAME[0]} 10 [:alnum:]\e[22m"
                echo "w2TZ8fHaRw"
                echo -e "$ \e[1m${FUNCNAME[0]} 10 [:alnum:][:punct:]\e[22m"
                echo "4pH?+40D):"
                echo 
                return 0
              ;;
            ...
            --selftest)
                ...
                echo "--> OK"
                echo -e "$ \e[1m${FUNCNAME[0]} 12 0-9\e[22m"
                stdout=$(${FUNCNAME[0]} 12 0-9); rc=$?
                echo $stdout
                if [[ $rc != 0 ]]; then echo "--> FAILED - exit code [$rc] instead of expected [0]."; return 1; fi
                regex="^[0-9]{12}$"
                if [[ ! "$stdout" =~ $regex ]]; then echo "--> FAILED - stdout [$stdout] does not match required pattern [[0-9]{12}]."; return 1; fi
                echo "--> OK"
                echo -e "$ \e[1m${FUNCNAME[0]} 8 a-zA-Z\e[22m"
                stdout=$(${FUNCNAME[0]} 8 a-zA-Z); rc=$?
                echo $stdout
                if [[ $rc != 0 ]]; then echo "--> FAILED - exit code [$rc] instead of expected [0]."; return 1; fi
                regex="^[a-zA-Z]{8}$"
                if [[ ! "$stdout" =~ $regex ]]; then echo "--> FAILED - stdout [$stdout] does not match required pattern [[a-zA-Z]{8}]."; return 1; fi
                echo "--> OK"
                echo -e "$ \e[1m${FUNCNAME[0]} 10 [:alnum:]\e[22m"
                stdout=$(${FUNCNAME[0]} 10 [:alnum:]); rc=$?
                echo $stdout
                if [[ $rc != 0 ]]; then echo "--> FAILED - exit code [$rc] instead of expected [0]."; return 1; fi
                regex="^[[:alnum:]]{10}$"
                if [[ ! "$stdout" =~ $regex ]]; then echo "--> FAILED - stdout [$stdout] does not match required pattern [[[:alnum:]]{10}]."; return 1; fi
                echo "--> OK"
                echo -e "$ \e[1m${FUNCNAME[0]} 10 [:alnum:][:punct:]\e[22m"
                stdout=$(${FUNCNAME[0]} 10 [:alnum:][:punct:]); rc=$?
                echo $stdout
                if [[ $rc != 0 ]]; then echo "--> FAILED - exit code [$rc] instead of expected [0]."; return 1; fi
                regex="^[[:alnum:][:punct:]]{10}$"
                if [[ ! "$stdout" =~ $regex ]]; then echo "--> FAILED - stdout [$stdout] does not match required pattern [[[:alnum:][:punct:]]{10}]."; return 1; fi
                echo "--> OK"
                echo "Testing function [${FUNCNAME[0]}]...DONE"
                return 0
              ;;
    ...
    if [[ $_LENGTH ]]; then
        if [[ ! "$_LENGTH" =~ ^-?[0-9]*$ ]]; then echo "Error: Value '$_LENGTH' for parameter LENGTH is not a numeric value."; return 1; fi
        if [[ $_LENGTH -lt 1 ]]; then echo "Error: Value '$_LENGTH' for parameter LENGTH is too low. Must be >= 1."; return 1; fi
        true
    else
        echo "Error: Parameter LENGTH must be specified."; return 1
    fi
    if [[ $_CHARS ]]; then
        true
    fi
