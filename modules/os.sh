    ...
    for arg in "$@"; do
        case $arg in
            ...
            --selftest)
                ...
                if [[ ! "$stdout" =~ $regex ]]; then echo -e "--> [31mFAILED[0m - stdout [$stdout] does not match required pattern ['hash' is available.].$hint"; return 1; fi
                echo "--> [32mOK[0m"
                echo -e "$ \033[1m$fn -v ls\033[22m"
                stdout=$($fn -v ls); rc=$?
                echo $stdout
                ...
                if [[ ! "$stdout" =~ $regex ]]; then echo -e "--> [31mFAILED[0m - stdout [$stdout] does not match required pattern ['ls' is available.].$hint"; return 1; fi
                echo "--> [32mOK[0m"
                echo -e "$ \033[1m$fn -v name-of-nonexistant-command\033[22m"
                ...
                if [[ ! "$stdout" =~ $regex ]]; then echo -e "--> [31mFAILED[0m - stdout [$stdout] does not match required pattern ['name-of-nonexistant-command' not found.].$hint"; return 1; fi
                echo "--> [32mOK[0m"
