...
function -command-exists() {
    ...
    for arg in "$@"; do
        case $arg in
            ...
            --help)
                echo "Usage: ${FUNCNAME[0]} [OPTION]... COMMAND"
                echo 
                ...
                echo 
                echo "Examples:"
                echo -e "$ \e[1m${FUNCNAME[0]} hash\e[22m"
                echo 
                echo -e "$ \e[1m${FUNCNAME[0]} -v hash\e[22m"
                echo "'hash' is available."
                echo -e "$ \e[1m${FUNCNAME[0]} -v ls\e[22m"
                echo "'ls' is available."
                echo -e "$ \e[1m${FUNCNAME[0]} -v name-of-nonexistant-command\e[22m"
            ...
            --selftest)
                ...
                echo "--> OK"
                echo -e "$ \e[1m${FUNCNAME[0]} -v hash\e[22m"
                stdout=$(${FUNCNAME[0]} -v hash); rc=$?
                echo $stdout
                if [[ $rc != 0 ]]; then echo "--> FAILED - exit code [$rc] instead of expected [0]."; return 1; fi
                regex="^'hash' is available.$"
                if [[ ! "$stdout" =~ $regex ]]; then echo "--> FAILED - stdout [$stdout] does not match required pattern ['hash' is available.]."; return 1; fi
                echo "--> OK"
                echo -e "$ \e[1m${FUNCNAME[0]} -v ls\e[22m"
                stdout=$(${FUNCNAME[0]} -v ls); rc=$?
                echo $stdout
                if [[ $rc != 0 ]]; then echo "--> FAILED - exit code [$rc] instead of expected [0]."; return 1; fi
                regex="^'ls' is available.$"
                if [[ ! "$stdout" =~ $regex ]]; then echo "--> FAILED - stdout [$stdout] does not match required pattern ['ls' is available.]."; return 1; fi
                echo "--> OK"
                echo -e "$ \e[1m${FUNCNAME[0]} -v name-of-nonexistant-command\e[22m"
                stdout=$(${FUNCNAME[0]} -v name-of-nonexistant-command); rc=$?
                echo $stdout
                if [[ $rc != 1 ]]; then echo "--> FAILED - exit code [$rc] instead of expected [1]."; return 1; fi
                regex="^'name-of-nonexistant-command' not found.$"
                if [[ ! "$stdout" =~ $regex ]]; then echo "--> FAILED - stdout [$stdout] does not match required pattern ['name-of-nonexistant-command' not found.]."; return 1; fi
                echo "--> OK"
    ...
    if [[ $_COMMAND ]]; then
        ...
        echo "Error: Parameter COMMAND must be specified."; return 1
    fi
...
function -pkg-installed() {
    ...
    for arg in "$@"; do
        case $arg in
            ...
            --help)
                echo "Usage: ${FUNCNAME[0]} [OPTION]... PACKAGE_NAME"
                echo 
    ...
    if [[ $_PACKAGE_NAME ]]; then
        ...
        echo "Error: Parameter PACKAGE_NAME must be specified."; return 1
    fi
